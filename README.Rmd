...
1. Initialize a pull request with `pr_init("prefix_you")` (you = your name).
1. _Copy_ (don't rename) each lesson as `prefix_you.Rmd` (you = your name).

```{r, echo=FALSE, comment = ""}
fs::dir_tree("R", regexp = "_you.Rmd")
```
...
1. Finish your pull request with `pr_finish()` (e.g. because it got merged).
1. Pause your pull request with `pr_pause()` (e.g. to initialize a new one).
